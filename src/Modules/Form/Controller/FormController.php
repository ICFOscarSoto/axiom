...
class FormController extends Controller{
  ...
  function datareceived($container,Request $request, $object){
            ...
            //dump($class);
            //dump($entityManager->getClassMetadata($class)->associationMappings);
            //dump($entityManager->getClassMetadata($class)->fieldMappings);

            //dump($request->query);
            //Iterate all simply field
            foreach($entityManager->getClassMetadata($class)->fieldMappings as $key=>$value){
              //dump($value["fieldName"]);

                //dump($request->query->get($value["fieldName"]));
                $data = null;
                $continue = true;
                  //dump($value);
                  switch  ($value["type"]){
          ...
          */
          function saveJSON($path){
            ...
            file_put_contents("/Users/juanpedro/Documents/HTTP/axiom/src/Utils/Form/example2Json",$json);
          }
          /**
          * Print form with Style theme
          */
          function printForm(){
          ...
          */
          ...
          function addCheckbox($type, $name, $label, $size, $value,$icon){
            ...
            $checkbox = "<div class='form-group col-sm-".$size."'>";
            $checkbox .= "<label class='col-sm-1 control-label'>".$label."</label>";
            $checkbox .= "<div class='col-sm-1'>";
            $checkbox .= "<div name='".$name."' id=''".$name."' class='make-switch'>";
          ...
          function addDateTime( $name, $label,$visibility, $value='',$enabled=true,$validation){
            ...
            $datePicker .= "<input type='text' name='".$name."-time' id='".$name."-time' class='form-control timepicker' data-template='dropdown' data-show-seconds='false' data-default-time='11:25 AM' data-show-meridian='false' data-minute-step='5' data-second-step='5' ".$enabledString." />";
            $datePicker .= "</div> </div> ";
