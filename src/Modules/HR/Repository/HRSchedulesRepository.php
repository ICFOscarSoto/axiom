...
class HRSchedulesRepository extends ServiceEntityRepository
{
        ...
        case 2: //Fijo
          $query="SELECT p.* FROM hrperiods p
                  ...
                  WHERE
                  	...
                  	p.".$dayWeek."=1 AND	p.active=1 AND p.deleted=0 AND s.active=1 AND s.deleted=0 AND sh.active=1 AND sh.deleted=0 LIMIT 1";
        ...
        case 3: //Rotativo
          ...
          $query="SELECT p.* FROM hrperiods p
                  ...
                  WHERE
                  	...
                  	(STR_TO_DATE(:DATE, '%Y-%m-%d') BETWEEN STR_TO_DATE(CONCAT(p.fromdate,'/', YEAR(CURDATE())), '%d/%m/%Y') AND STR_TO_DATE(CONCAT(p.todate,'/', YEAR(CURDATE())), '%d/%m/%Y')) AND
                  	(STR_TO_DATE(:TIME, '%H:%i:%s') BETWEEN STR_TO_DATE(p.start, '%H:%i:%s') AND STR_TO_DATE(p.end, '%H:%i:%s')) AND
                  	p.".$dayWeek."=1 AND	p.active=1 AND p.deleted=0 AND s.active=1 AND s.deleted=0 LIMIT 1";
    ...
    public function hoursDayWork($worker, $date){
      $schedule=$worker->getSchedule();
      if($schedule==null) return false;
      //Get day of week
      $dayWeek=date("l", strtotime($date));
      ...
      switch($schedule->getType()){
        ...
        case 2: //Fijo
          $query="SELECT IFNULL(SUM(ROUND(TIME_TO_SEC(IF(TIMEDIFF(p.END, p.START)<0, TIMEDIFF('24:00:00',TIMEDIFF(p.START, p.END)),TIMEDIFF(p.END, p.START)))/3600,2)),0) hours FROM hrperiods p
                  	LEFT JOIN hrshifts s ON p.shift_id=s.id
                  	LEFT JOIN hrschedules sh ON s.schedule_id=sh.id
                  	LEFT JOIN hrworkers w ON sh.id=w.schedule_id
                  WHERE
                  	w.id=2 AND
                  	(STR_TO_DATE(:DATE, '%Y-%m-%d') BETWEEN STR_TO_DATE(CONCAT(p.fromdate,'/', YEAR(CURDATE())), '%d/%m/%Y') AND STR_TO_DATE(CONCAT(p.todate,'/', YEAR(CURDATE())), '%d/%m/%Y')) AND
                  	p.".$dayWeek."=1 AND	p.active=1 AND p.deleted=0 AND s.active=1 AND s.deleted=0 AND sh.active=1 AND sh.deleted=0";
                    $params=['WORKERID' => $worker->getId(),
                             'DATE' => $date];
          $result=$this->getEntityManager()->getConnection()->executeQuery($query, $params)->fetch();
          return $result["hours"];
