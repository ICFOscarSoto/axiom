...
class HelperMercateo {
  ...
  public function convertToOrder($xml, $doctrine){
    ...
    $order['CodCliente']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->PARTY_ID->__toString();
    $order['Nombre']=mb_strtoupper($xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->NAME->__toString(),'utf-8');
    $order['Apellido1']='';
    $order['Apellido2']='';
    ...
    $order['CP']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->ZIP->__toString();
    $order['Poblacion']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->CITY->__toString();
    $order['Provincia']=$state;
    $order['Contacto']='';
    ...
    $order['Telefono']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->PHONE->__toString();
    $order['CodClientePagadorPS']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->INVOICE_PARTY->PARTY->PARTY_ID->__toString();
    $order['NombrePagador']=mb_strtoupper($xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->INVOICE_PARTY->PARTY->ADDRESS->NAME->__toString(), 'utf-8');
    ...
    $order['CPEnvio']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->SHIPMENT_PARTIES->DELIVERY_PARTY->PARTY->ADDRESS->ZIP->__toString();
    $order['PoblacionEnvio']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->SHIPMENT_PARTIES->DELIVERY_PARTY->PARTY->ADDRESS->CITY->__toString();
    $order['ProvinciaEnvio']=$stateShipment;
    $order['CodPedidoPS']=preg_replace("/[^A-Za-z0-9 ]/", '', $xml->ORDER_HEADER->ORDER_INFO->ORDER_ID->__toString());
    $order['RefPedidoPS']=preg_replace("/[^A-Za-z0-9 ]/", '', $xml->ORDER_HEADER->ORDER_INFO->ORDER_ID->__toString());
    $order['Observaciones']='';
    $order['EmpresaPagador']=mb_strtoupper($xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->NAME->__toString(),'utf-8');
    $order['EmpresaEnvios']=mb_strtoupper($xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->SHIPMENT_PARTIES->DELIVERY_PARTY->PARTY->ADDRESS->NAME->__toString(),'utf-8');
    $order['Email']=$xml->ORDER_HEADER->ORDER_INFO->ORDER_PARTIES->BUYER_PARTY->PARTY->ADDRESS->EMAIL->__toString();
    $lines=[];
