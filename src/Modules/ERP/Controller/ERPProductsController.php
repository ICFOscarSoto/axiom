	 ...
	 */
	 ...
	 public function generateQR(RouterInterface $router,Request $request){
		...
		if ($request->query->get('name',null)) {
		 ...
		 $repositoryUsers=$this->getDoctrine()->getRepository(GlobaleUsers::class);
		 $params["user"]=$repositoryUsers->findOneBy(["active"=>1, "deleted"=>0, "id"=>$request->query->get('userId', null)]);
		 $transfer=$repositoryTransfers->findOneBy(["hash"=>$request->query->get('hash',null), "active"=>1, "deleted"=>0]);
