...
class ERPPurchasesBudgetsController extends Controller
{
	private $module='ERP';
	private $prefix='PRE';
	private $class=ERPPurchasesBudgets::class;
	private $classLines=ERPPurchasesBudgetsLines::class;
	private $utilsClass=ERPPurchasesBudgetsUtils::class;

	/**
	 * @Route("/{_locale}/ERP/purchasesbudgets/form/{id}", name="ERPPurchasesBudgetsForm", defaults={"id"=0}))
	 */
	public function ERPPurchasesBudgetsForm($id, RouterInterface $router,Request $request)
	{
		...
		$documentRepository=$this->getDoctrine()->getRepository(ERPPurchasesBudgets::class);
		$documentLinesRepository=$this->getDoctrine()->getRepository(ERPPurchasesBudgetsLines::class);
		...
		if($id!=0){
			...
			$documentLines=$documentLinesRepository->findBy(["purchasesbudget"=>$document, "active"=>1,"deleted"=>0]);
		...
		if($document->getInPurchasesOrder()!=null)
			array_push($warnings, "Este presupuesto ya esta asociado al pedido número <a href='".$this->generateUrl("ERPPurchasesOrdersForm",["id"=>$document->getInPurchasesOrder()->getId()])."'>".$document->getInPurchasesOrder()->getCode()."</a>, puede editar este presupuesto haciendo click <a href=\"javascript:unlockFields();\">aquí</a> aunque se aconseja generar un nuevo presupuesto o editar directamente el pedido.");


		$new_breadcrumb=["rute"=>null, "name"=>$id?"Editar":"Nuevo", "icon"=>$id?"fa fa-edit":"fa fa-plus"];
		$breadcrumb=$menurepository->formatBreadcrumb('genericindex','ERP','PurchasesBudgets');
		array_push($breadcrumb,$new_breadcrumb);
		if ($this->get('security.authorization_checker')->isGranted('ROLE_USER')) {
			return $this->render('@ERP/purchasesbudgets.html.twig', [
				'moduleConfig' => $config,
				'controllerName' => 'categoriesController',
				'interfaceName' => 'PurchasesBudgets',
				'optionSelected' => 'genericindex',
				'optionSelectedParams' => ["module"=>"ERP", "name"=>"PurchasesBudgets"],
				...
				'id' => $id,
				'documentType' => 'purchases_budget',
				'documentPrefix' => $this->prefix,
				'document' => $document,
				'documentLines' => $documentLines,
				'documentReadonly' => $document->getInPurchasesOrder()!=null?true:false,
				'errors' => $errors,
				'warnings' => $warnings,
	...
	/**
	 * @Route("/{_locale}/ERP/purchasesbudgets/data/{id}", name="dataERPPurchasesBudgets", defaults={"id"=0}))
	 */
	public function data($id, RouterInterface $router,Request $request){
		...
		if(!$document){
			$document=new ERPPurchasesBudgets();
		...
		foreach ($fields->lines as $key => $value) {
			...
			if(!$line ){
				$line=new ERPPurchasesBudgetsLines();
	...
	/**
	 * @Route("/{_locale}/ERP/purchasesbudgets/document/{id}/{mode}", name="ERPPurchasesBudgetsPrint", defaults={"id"=0, "mode"="print"}))
	 */
	public function ERPPurchasesBudgetsPrint($id, $mode, RouterInterface $router,Request $request){
		$this->denyAccessUnlessGranted('IS_AUTHENTICATED_REMEMBERED');
		...
		$reportsUtils = new ERPPurchasesBudgetsReports();
		switch($mode){
			case "email":
	...
	/**
	 * @Route("/{_locale}/ERP/purchasesbudgets/createorder/{id}", name="ERPOrderFromPurchasesBudgets", defaults={"id"=0}))
	 */
	public function ERPOrderFromPurchasesBudgets($id, RouterInterface $router,Request $request){
		$this->denyAccessUnlessGranted('IS_AUTHENTICATED_REMEMBERED');
		$documentRepository=$this->getDoctrine()->getRepository(ERPPurchasesBudgets::class);
		$documentLinesRepository=$this->getDoctrine()->getRepository(ERPPurchasesBudgetsLines::class);
