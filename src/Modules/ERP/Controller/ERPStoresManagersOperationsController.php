    ...
    /**
		 ...
		 public function createVendingMachineOperations($id, $channel_num, $nfcid, Request $request){
			...
			if($channel->getProduct()){
				    ...
				    $result = file_get_contents($request->getSchemeAndHttpHost().'/api/ERP/storesmanagers/vendingmachines/replenishment/channels/add/'.$channel->getId().'/'.($channel->getMultiplier()?$channel->getMultiplier():1), false, $context);
						...
						$channel=$repositoryVendingMachinesChannels->findOneBy(["vendingmachine"=>$vendingmachine,"channel"=>$channel_num,"active"=>1,"deleted"=>0]);
						//Informamos de la venta por debajo de stock en el canal de discord
						...
						$postdata = http_build_query([
										'from' => '14',
										'to' => $vendingmachine->getReplenishmentnotifyaddress(),
										'cc' => '',
										'bcc' =>'',
										'subject' =>'Error en la máquina '.$vendingmachine->getName().' operacion por debajo de 0.' ,
										'files' => '{}',
										'text_content' =>'',
										'html_content' => $msg
									]);
	  ...
	  */
	  public function deleteOperation($id){
		 ...
		 } else if ($operation->getVendingmachine()==null)
		 		return new JsonResponse(["result"=>-4, "text"=> "No existen ubicación ni máquina en la operacion"]);
			 else {
				foreach($operationsLines as $line){
					$product = $line->getProductvariant()->getProduct();
					$channel=$channelRepository->findOneBy(["vendingmachine"=>$operation->getVendingmachine(), "product"=>$product, "active"=>1,"deleted"=>0]);
					if(!$channel) continue;
					$channel->setQuantity($channel->getQuantity()+($line->getQuantity()));
					$this->getDoctrine()->getManager()->persist($channel);
					$this->getDoctrine()->getManager()->flush();
