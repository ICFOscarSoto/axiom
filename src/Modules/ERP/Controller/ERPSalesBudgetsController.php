...
use App\Modules\ERP\Utils\ERPSalesBudgetsUtils;
...
use App\Modules\ERP\Reports\ERPSalesBudgetsReports;
use App\Modules\Security\Utils\SecurityUtils;

class ERPSalesBudgetsController extends Controller
{
	private $module='ERP';
	private $prefix='PRE';
	private $class=ERPSalesBudgets::class;
	private $classLines=ERPSalesBudgetsLines::class;
	private $utilsClass=ERPSalesBudgetsUtils::class;

	/**
	 * @Route("/{_locale}/ERP/salesbudgets/form/{id}", name="ERPSalesBudgetsForm", defaults={"id"=0}))
	 */
	public function ERPSalesBudgetsForm($id, RouterInterface $router,Request $request)
	{
		...
		$breadcrumb=$menurepository->formatBreadcrumb('genericindex','ERP','SalesBudgets');
		array_push($breadcrumb,$new_breadcrumb);
		if ($this->get('security.authorization_checker')->isGranted('ROLE_USER')) {
			return $this->render('@ERP/salesbudgets.html.twig', [
				'moduleConfig' => $config,
				'controllerName' => 'categoriesController',
				'interfaceName' => 'SalesBudgets',
				'optionSelected' => 'genericindex',
				'optionSelectedParams' => ["module"=>"ERP", "name"=>"SalesBudgets"],
	...
	/**
	 * @Route("/{_locale}/ERP/salesbudgets/data/{id}", name="dataERPSalesBudgets", defaults={"id"=0}))
	 */
	...
	/**
	 * @Route("/{_locale}/ERP/salesbudgets/document/{id}/{mode}", name="ERPSalesBudgetsPrint", defaults={"id"=0, "mode"="print"}))
	 */
	public function ERPSalesBudgetsPrint($id, $mode, RouterInterface $router,Request $request){
		$this->denyAccessUnlessGranted('IS_AUTHENTICATED_REMEMBERED');
		...
		$reportsUtils = new ERPSalesBudgetsReports();
		switch($mode){
			case "email":
	...
	/**
	 * @Route("/{_locale}/ERP/salesbudgets/createorder/{id}", name="ERPOrderFromSalesBudgets", defaults={"id"=0}))
	 */
	public function ERPOrderFromSalesBudgets($id, RouterInterface $router,Request $request){
		$this->denyAccessUnlessGranted('IS_AUTHENTICATED_REMEMBERED');
		$documentRepository=$this->getDoctrine()->getRepository(ERPSalesBudgets::class);
		$documentLinesRepository=$this->getDoctrine()->getRepository(ERPSalesBudgetsLines::class);
