...
class ERPSupplierCommentLinesRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function getCommentByBuyOrder($supplier){
      ...
      $query="SELECT GROUP_CONCAT(concat(comment, if(start is null,'',concat(' - Desde ',date_format(start,'%d/%m/%Y'))), if(end is null,'',concat(' - Hasta ',date_format(end,'%d/%m/%Y')))) order by start, id separator '\\n') as comment
