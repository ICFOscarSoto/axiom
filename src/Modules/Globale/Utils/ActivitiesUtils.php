...
class ActivitiesUtils
{
  public function formatEditor($user, $obj, $request, $controller, $doctrine, $name, $icon){
    ...
    $breadcrumb=$menurepository->formatBreadcrumb('activities');
    ...
    return ['template'=>'@Globale/genericform.html.twig', 'vars'=>array(
        ...
        'interfaceName' => 'Activities',
        'optionSelected' => 'activities',
        'menuOptions' =>  $menurepository->formatOptions($userdata["roles"]),
        'breadcrumb' =>  $breadcrumb,
        'userData' => $userdata,
        'form' => ["form" => $form->createView(),"template" => json_decode(file_get_contents (dirname(__FILE__)."/../Forms/Activities.json"),true)]
    )];
  }
  ...
  public function formatList($user){
    $list=[
      ...
      'orderDirection' => 'ASC',
      'tagColumn' => 3,
      'fields' => json_decode(file_get_contents (dirname(__FILE__)."/../Lists/Activities.json"),true),
      'fieldButtons' => json_decode(file_get_contents (dirname(__FILE__)."/../Lists/ActivitiesFieldButtons.json"),true),
      'topButtons' => json_decode(file_get_contents (dirname(__FILE__)."/../Lists/ActivitiesTopButtons.json"),true)
    ];
    return $list;
  }
}
