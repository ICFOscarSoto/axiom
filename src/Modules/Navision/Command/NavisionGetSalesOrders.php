      ...
      /*  if($object["web"]==1)
        ...
        if ($obj==null) {
         ...
         if($customer==NULL AND $object["web"]==1) {
           ...
           $customer=$repositoryCustomers->findOneBy(["code"=>$order_array["id_customer"]]);
           */
        //   $output->writeln(' VAT: '.$object["vat"]);
        //   $output->writeln(' email: '.$object["email"]);
                ...
                if(empty($xml_addresses->addresses->address)){
                  ...
                  foreach($xml_addresses->addresses->address as $address)
                  {
                    if(!$exist_invoice_address)
                    {
                        ...
                        {
                      //    print_r($inv_add_array["address2"]);
