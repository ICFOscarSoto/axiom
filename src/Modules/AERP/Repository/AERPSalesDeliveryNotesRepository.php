...
class AERPSalesDeliveryNotesRepository extends ServiceEntityRepository
{
    ...
    {
      $query="SELECT IFNULL(MAX(number)+1,1) AS number FROM aerpsales_delivery_notes WHERE company_id=:company AND serie_id=:serie AND financialyear_id=:financialyear AND active=1 AND deleted=0";
