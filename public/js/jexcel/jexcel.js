...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        /*
          Replace url
        */
        obj.replaceUrl = function(url, y){
          var url_new = '';
          var error = false;
          if (url!=''){
            var aurl_new = url.split('/');
            for (var i=0; i<aurl_new.length; i++){
              ...
              // Celda de esta tabla
              if (value.substr(0,3)=='#c|'){
                var field_cell  = value.substr(3);
                var afield_cell = field_cell.split('|');
                var cell_y      = parseInt(y);
                if (afield_cell.length==1 || afield_cell.length==2){
                  var cell_x = afield_cell[0];
                  if (afield_cell.length==2)
                    cell_y = parseInt(afield_cell[1]);
                  if (cell_x >>> 0 === parseFloat(cell_x))
                    value = obj.getValueFromCoords(cell_x, cell_y);
                  else
        ...
        */
        ...
        /**
         * Get the value from a coords column key
         *
         * @param string  key
         * @param int y
         * @return string value
         */
        obj.getValueFromKey = function(key, y, processedValue) {
            var value = null;
            // Calculate x column for key
            var x = obj.getColumnKey(key+'');
            if (x != null  && x!=-1 && y != null) {
        ...
        /**
          Get column position on key column
        */
        obj.getColumnKey = function(key) {
            var column = -1;
            for (var i = 0; i < obj.options.columns.length && column==-1; i++) {
                if (obj.options.columns[i].name.toUpperCase()==key.toUpperCase())
                  column = i;
            }
            return column;
        }
        ...
        */
        ...
        /**
          Replace  token {x|y} for cell
        */
        obj.replaceFormula = function(value, y) {
          var regexp = /[{][A-Za-z0-9_\-\|]+[}]/g;
          if (typeof(value) == 'string'){
            var avalue = value.match(regexp);
            if (avalue!=null){
              for(var i=0; i<avalue.length; i++){
                ...
                key = key.split('|');
                if (key.length>0)
                  x = obj.getColumnKey(key[0]);
                if (key.length==2)
                  y = parseInt(key[1])-1;
                var cell = jexcel.getColumnNameFromId([x, y]);
                value = value.replace(token, cell);
              }
            }
          }
          return value;
         ...
         * Show column
         */
        obj.showColumnKey = function(key) {
            var colNumber = obj.getColumnKey(key);
            obj.headers[colNumber].style.display = '';
            obj.colgroup[colNumber].style.display = '';
            for (var j = 0; j < obj.options.data.length; j++) {
                obj.records[j][colNumber].style.display = '';
            }
            obj.options.columns[colNumber].options.hide = false;
            if (!obj.options.loadinit){
              $('#'+obj.options.name).find('.jexcel_tab_link.saveview.disabled').each(function(){ $(this).removeClass('disabled'); });
              $('#'+obj.options.name).find('.jexcel_tab_link.tabnew').each(function(){ $(this).addClass('disabled'); });
            }
        }
         ...
         * Hide column
         */
        obj.hideColumnKey = function(key) {
            var colNumber = obj.getColumnKey(key);
            ...
            obj.options.columns[colNumber].options.hide = true;
            if (!obj.options.loadinit){
              $('#'+obj.options.name).find('.jexcel_tab_link.saveview.disabled').each(function(){ $(this).removeClass('disabled'); });
              $('#'+obj.options.name).find('.jexcel_tab_link.tabnew').each(function(){ $(this).addClass('disabled'); });
            }
         ...
         * @return void
         */
        obj.moveColumnKey = function(keyo, keyd) {
            var o = obj.getColumnKey(keyo);
            var d = obj.getColumnKey(keyd);
        ...
        obj.createCell = function(i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    // ICF
                    td.innerText = obj.getDropDownKeyValue(value);
            ...
            // ICF
            if (obj.options.columns[i].options.hide !== undefined && obj.options.columns[i].options.hide)
              td.style.display = 'none';
            // ICF
            return td;
        }
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        //ICF
                        if (typeof(obj.options.columns[x].options)  !== undefined){
                          for (const key in obj.options.columns[x].options) {
                            options[key] = obj.options.columns[x].options[key];
                          }
                        }
                        //ICF
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && ! force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        obj.options.data[y][x] = value;
                        // ICF
                        obj.records[y][x].innerText = obj.getDropDownKeyValue(value);
                        //obj.records[y][x].innerText = obj.getDropDownValue(y, value);
                        // ICF
                    } else if (obj.options.columns[x].type == 'calendar') {
                        // Update calendar
    ...
    jexcel.tabs = function(tabs, result) {
        var instances = [];
        // ICF
        var allowtabnew = true;
        var allowsaveview = true;
        if (typeof(result[0].allowtabnew) != 'undefined')
          allowtabnew = result[0].allowtabnew;
        if (typeof(result[0].allowsaveview) != 'undefined')
          allowsaveview = result[0].allowsaveview;
        // ICF
        ...
        if (allowtabnew){
          ...
          linkb[b].onclick = function() {
            ...
            if (this.getAttribute('class').indexOf('disabled')==-1 && headersd.length>0){
              ...
              if (tab!=null){
                var tabnew = window.prompt('Cambiar nombre de la pestaña', tab.textContent);
                if (tabnew!=null && tabnew!=''){
                  tab.innerHTML = tabnew;
                  $('#'+result[0].name).find('.jexcel_tab_link.saveview.disabled').each(function(){ $(this).removeClass('disabled'); });
                }
              }
            }else {
                ...
                for(var v=0; v<headers.children.length; v++){
                  ...
                  for(var c=0; c<columns.length; c++){
                    ...
                    if (typeof(columns[c]) != 'undefined' &&
                        typeof(columns[c].name) != 'undefined' &&
                        ((typeof(columns[c].options) == 'undefined') ||
                         (typeof(columns[c].options) != 'undefined' && (typeof(columns[c].options.hide) == 'undefined')) ||
                         (typeof(columns[c].options) != 'undefined' && (typeof(columns[c].options.hide) != 'undefined') && !columns[c].options.hide)
                       )){
                      column['name'] = columns[c].name;
