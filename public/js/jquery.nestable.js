...
;(function($, window, document, undefined)
{
	...
	var defaults = {
			...
			emptyClass      : 'dd-empty',
			expandBtnHTML   : '<button data-action="expand" class="btn-nestable" type="button">Expand</button>',
			collapseBtnHTML : '<button data-action="collapse" class="btn-nestable" type="button">Collapse</button>',
			group           : 0,
			maxDepth        : 5,
			threshold       : 20,
			/* callback */
	...
	Plugin.prototype = {
		...
		setParent: function(li)
		{
			if (li.children(this.options.listNodeName).length) {
				...
				li.find(".dd-noExpand").remove();
			}else{
				if(li.find('.dd-noExpand').length === 0 && li.find('.btn-nestable').length===0){
					li.prepend('<div class="dd-noExpand"></div>');
				}
			}
			li.children('[data-action="expand"]').hide();
		},
